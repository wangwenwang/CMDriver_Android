...
import com.chenmaunion.app.cmdriver.viewmodel.DBModel_Notify;
 ...
 */
public class DBNotifyAdapter extends RecyclerView.Adapter<DBNotifyAdapter.BindingViewHolder> {
   ...
   // public Context mcontext;
    public DBNotifyAdapter() {
        this.notifies = new ObservableArrayList<>();
    //    mcontext=context;
    }

    public void setData(ObservableArrayList<DBModel_Notify> notifyList){
        this.notifies = notifyList;
    ...
    public BindingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType==TYPE_ITEM){
            binding=DataBindingUtil.inflate(
                    LayoutInflater.from(parent.getContext()), R.layout.item_recycler_databinding,parent,false);
        }else {
            binding=DataBindingUtil.inflate(
    ...
    public void onBindViewHolder(BindingViewHolder holder, int position) {
        if (getItemViewType(position)==TYPE_ITEM){
            DBModel_Notify notify=notifies.get(position);
            ViewDataBinding contentbing= holder.getBinding();
            contentbing.setVariable(BR.model,notify);
            contentbing.executePendingBindings();
        }else {
    ...
    public int getItemCount() {
        return notifies.size()+1;//预加上底部FootView
    }
    ...
    public class NotifyViewHolder extends BindingViewHolder{
        public NotifyViewHolder(View itemView) {
            super(itemView);
        }
    }
    public class FootViewHolder extends BindingViewHolder{
